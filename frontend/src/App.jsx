...
import { fetchSchedule, fetchDrivers, fetchTelemetry, fetchDriverLaps } from "./api";
...
const buildColorMap = (data) => {
  ...
  data.forEach((d, index) => {
    let color = `#${d.TeamColor}`.toLowerCase();
  ...
  return (
    <div className="app">
      ...
      <div className="view-tabs">
        <button
          className={`view-tab ${activeView === "telemetry" ? "active" : ""}`}
          onClick={() => setActiveView("telemetry")}
        >
          Telemetry
        </button>
        <button
          className={`view-tab ${activeView === "replay" ? "active" : ""}`}
      ...
      {/* Controls — shared between both tabs */}
      <div className="controls">
        ...
        {/* Compare button only on telemetry tab */}
            ...
            <>
              ...
              <div className="charts-section">
                ...
                {["Speed", "Throttle", "Brake", "RPM", "nGear"].map(ch => (
                  <TelemetryChart key={ch} data={telemetry} channel={ch}
                    driverColors={driverColors} onHover={onHover} sectors={telemetry.sectors} />
                ))}
              </div>
            </>
          )}
      ...
      {/* ── REPLAY VIEW ── */}
      {activeView === "replay" && (
        <RaceReplay year={year} round={selectedRound} session={session} />
      )}
    </div>
  );
}